...
function loadState() {
  ...
  return {
    mapel: DEFAULT_MAPEL,
    jadwal: {},   // { senin: [{id, mapelId, jamMulai, jamSelesai, guru, ruangan}] }
    tugas: [],    // [{id, judul, mapelId, guru, deadline, tipe, catatan, selesai, createdAt}]
...
function saveState() {
  ...
  // Sync ke Firebase kalau user sudah login
  if (window._fb) {
...
function generateTimeSlots() {
  ...
  const slots = [];
  for (let t = start; t < end; t += durasiSlot) {
...
function renderJadwal() {
  ...
  dayJadwal.forEach(item => {
    ...
    el.style.cssText = `top:${topOffset}px; height:${Math.max(height,32)}px; background:${mapel.warna}22; border-left-color:${mapel.warna}; animation: scaleIn 0.25s cubic-bezier(0.34,1.56,0.64,1) both;`;
    el.innerHTML = `
      <div class="item-name">${mapel.ikon} ${mapel.nama}</div>
      <div class="item-time">${item.jamMulai} – ${item.jamSelesai}</div>
      ${item.guru ? `<div class="item-guru">👤 ${item.guru}</div>` : ''}
      ${item.ruangan ? `<div class="item-guru">🚪 ${item.ruangan}</div>` : ''}
...
function openModalMapel(editId = null) {
  ...
  sel.innerHTML = state.mapel.map(m => `<option value="${m.id}">${m.ikon} ${m.nama}</option>`).join('');
...
function saveModalMapel() {
  ...
  if (!mapelId || !jamMulai || !jamSelesai) {
    showToast('Lengkapi data jadwal!', 'error'); return;
  }
  if (timeToMinutes(jamMulai) >= timeToMinutes(jamSelesai)) {
    showToast('Jam mulai harus sebelum jam selesai!', 'error'); return;
  }
...
function renderTugas() {
  ...
  // Sort: belum selesai dulu, lalu by deadline
  items.sort((a, b) => {
  ...
  list.innerHTML = items.map(t => {
    ...
    const deadlineStr = t.deadline ? new Date(t.deadline).toLocaleDateString('id-ID', { day:'numeric', month:'short', year:'numeric' }) : '-';
    return `
      <div class="tugas-item ${t.selesai ? 'selesai' : ''}" data-id="${t.id}">
        ...
        <div class="tugas-content">
          ...
          <div class="tugas-meta">
            ${mapel ? `<span class="tugas-meta-item" style="color:${mapel.warna}">${mapel.ikon} ${mapel.nama}</span>` : ''}
            ${t.guru ? `<span class="tugas-meta-item">👤 ${t.guru}</span>` : ''}
          ...
          ${t.catatan ? `<div style="font-size:12px;color:var(--text3);margin-top:6px">📌 ${t.catatan}</div>` : ''}
        </div>
...
function toggleTugas(id) {
  ...
  if (t) {
    ...
    saveState();
    renderTugas();
    // Auto-sync status ke Sheets jika terhubung
...
function renderSettings() {
  ...
  list.innerHTML = state.mapel.map(m => `
    <div class="mapel-item" style="animation: slideUp 0.25s ease both">
      ...
      <span class="mapel-name">${m.nama}</span>
...
// Kode Apps Script yang akan di-copy user
...
function getTugasData() {
  ...
  return sheet.getRange(2,1,lastRow-1,TUGAS_HEADERS.length).getValues()
    ...
    .map(r => ({
      ...
      guru: r[3].toString(), deadline: r[4] ? Utilities.formatDate(new Date(r[4]),'Asia/Jakarta','yyyy-MM-dd') : '',
...
function showSheetsStatus(msg, type, elId = 'sheetsStatus') {
  ...
  if (!msg) { el.className = 'status-msg'; return; }
...
async function pushToSheets() {
  ...
  try {
    ...
    const res = await fetch(url, {
      method: 'POST',
      body: JSON.stringify({ action: 'syncTugas', data: payload })
    });
    const json = await res.json();
    if (!json.ok) throw new Error(json.error);
...
// ---- Auto-sync status saat toggle tugas ----
...
function init() {
  ...
  // Tambah Mapel (Jadwal) - mobile + desktop buttons
  ...
  document.getElementById('modalMapel').addEventListener('click', (e) => {
    if (e.target === e.currentTarget) closeModalMapel();
  });

  // Tambah Tugas - mobile + desktop
  ...
  document.getElementById('modalTugas').addEventListener('click', (e) => {
    if (e.target === e.currentTarget) closeModalTugas();
  });
  ...
  document.getElementById('modalMapelBaru').addEventListener('click', (e) => {
    if (e.target === e.currentTarget) closeModalMapelBaru();
  });

  // Settings - Save Jam (autosave on change)
  ...
  document.addEventListener('keydown', (e) => {
    if (e.key === 'Escape') {
      closeModalMapel(); closeModalTugas(); closeModalMapelBaru();
    }
  });

  // ===== FIREBASE AUTH HANDLERS =====
  // Dipanggil oleh Firebase module saat login berhasil dan data cloud tersedia
  ...
  // Dipanggil saat ada perubahan dari device lain (realtime Firestore)
